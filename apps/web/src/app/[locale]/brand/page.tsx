...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonicalUrl = `${siteUrl}${prefix}/brand`
...
export default async function BrandShowroomPage({ params }: Props) {
  const { locale } = await params
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      {
        '@type': 'WebPage',
        '@id': `${siteUrl}${prefix}/brand#webpage`,
        url: `${siteUrl}${prefix}/brand`,
        name: locale === 'ko'
          ? '픽셀리프 브랜드 아이덴티티 | 23.5도의 철학'
          : 'Pixelyf Brand Identity | The Philosophy of 23.5°',
        description: locale === 'ko'
          ? '픽셀(Pixel)과 삶(Life)이 결합된 픽셀리프의 브랜드 세계관 — 23.5도의 기울어진 프레임, The Soul AI 아바타 기술, 그리고 디지털 영혼이 연결되는 우주적 철학을 소개합니다.'
          : "Discover Pixelyf's brand universe — the philosophy of 23.5°, The Soul AI avatar technology, and the cosmic vision of connecting digital souls across the globe.",
        isPartOf: {
          '@type': 'WebSite',
          '@id': `${siteUrl}/#website`,
          name: 'Pixelyf',
      ...
      {
        '@type': 'FAQPage',
        '@id': `${siteUrl}${prefix}/brand#faqpage`,
        mainEntity: [
          {
            '@type': 'Question',
  ...
  return (
    <>
      ...
      <div className="sr-only" aria-hidden="true">
        <h1>{getTranslationWithFallback('namingTitle', locale === 'ko' ? '픽셀리프(Pixelyf) 브랜드 아이덴티티' : 'Pixelyf Brand Identity')}</h1>
