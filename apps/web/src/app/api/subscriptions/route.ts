...
const SUBSCRIPTION_COST = 0 // 기본 월 구독 비용 (스타더스트)
...
export async function POST(request: Request) {
  try {
    ...
    if (SUBSCRIPTION_COST === 0) {
      // [무료 구독 개편] 스타더스트 트랜잭션 없이 즉시 구독 레코드 생성/업데이트
      const userBalance = await prisma.user.findUnique({
        where: { id: user.id },
        select: { stardust_balance: true }
      })
      newBalance = userBalance?.stardust_balance || 0
      ...
      // 스타더스트 차감 (Prisma 트랜잭션) — 유료 구독용 레거시 코드 원형 보존
